<x-admin.layouts.app :breadcrumb="$breadcrumb">
    <x-slot:pageTitle>{{ __('States') }}</x-slot>
    ...
    <div x-data="modalDelete">
        <div class="card">
            <div class="card-body">
                <table
                    data-toggle="table"
                    data-url="{{ route('admin.states.index.data') }}"
        ...
        <x-admin.components.modal name="delete">
            <x-slot:modalTitle>{{ __('Delete State?') }}</x-slot>
            <p>{{ __('Delete state:') }} <span class="font-weight-bold" x-text="itemName"></span></p>
            <p class="mb-0">{{ __('All related data will be deleted (cities, hotels, etc.)') }}</p>
            <x-slot:modalFooter>
