<x-admin.layouts.app :breadcrumb="$breadcrumb">
    ...
    <div x-data="permission">
        <form action="{{ route('admin.groups.member-permissions.update', ['group' => $group]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row" x-show="isRestricted == 'Y'" x-cloak>
                @foreach (config('permissions.member') as $subPermissionName => $subPermissions)
                    <div class="col-12 col-lg-6 d-flex">
                        <div class="card flex-grow-1">
                            <div class="card-header">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" name="cat[]" value="{{ $subPermissionName }}" id="checkboxCat{{ Str::studly($subPermissionName) }}" @change="categoryChecked" @checked($permissionsCategory[$subPermissionName] || $group->is_member_restricted == 'N')>
                            ...
                            <div class="card-body">
                                @foreach ($subPermissions as $permissionName => $permissionCode)
                                    <div>
                                        <div class="custom-control custom-switch">
                                            <input type="checkbox" class="custom-control-input" name="permissions[]" value="{{ $permissionCode }}" id="checkbox{{ Str::studly($permissionCode) }}" data-parent="{{ $subPermissionName }}" @change="permissionChecked" @checked(in_array($permissionCode, $permissions) || $group->is_member_restricted == 'N')>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('permission', () => ({
                    isRestricted: '{{ $group->is_member_restricted }}',
                    categoryChecked(e) {
